 ...
 * @param optionsJson
 */
window.reportWatermark = function(options) {
  if(!options) {
    return;
  }
  ...
  // 颜色格式转换
  let watermarkColor = `rgb(${options.color[0]}, ${options.color[1]}, ${options.color[2]})`;
  options = {
    watermark_txt: content,
    watermark_width: 120,
    watermark_height: 50,
    watermark_color: watermarkColor,
    watermark_fontsize: options.size + 'px',
    watermark_alpha: options.alpha,
    watermark_angle: options.tiltAngle,
    watermark_x_space: options.hSpace,
    watermark_y_space: options.vSpace,
    //watermark_parent_node: "keep_report_table",
  }
