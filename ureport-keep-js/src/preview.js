...
$(document).ready(function(){
    ...
    $('.ureport-print').click(function(){
        ...
        const url=window._server+'/html/loadPrintPages'+urlParameters;
        showLoading();
        $.ajax({
            url,
            type:'POST',
            success:function(data){
                ...
                $.get(window._server+'/html/loadPagePaper'+urlParameters,function(paper){
                    hideLoading();
...
window.buildPaging=function(pageIndex,totalPage){
    ...
    pageSelector.change(function(){
        ...
        let url=window._server+`/html/preview${parameters}&_i=${$(this).val()}`;
    ...
    if(pageIndex>1){
        let url=window._server+`/html/preview${parameters}&_i=${pageIndex-1}`;
...
function _refreshData(second){
    ...
    let url=window._server+`/html/loadData${params}`;
...
window.submitSearchForm=function(file,customParameters){
    ...
    const parameters=window.buildLocationSearchParameters('_i');
    let url=window._server+"/html/loadData"+parameters;
    const pageSelector=$(`#pageSelector`);
