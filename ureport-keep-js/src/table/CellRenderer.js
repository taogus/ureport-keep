...
export function afterRenderer(td,row,col,prop,value,cellProperties){
    ...
    if(valueType==='dataset'){
        ...
        const imagePath=window._server+'/static/ureport-asserts/icons/image.svg';
    ...
    if(cellDef.expand === 'Down'){
        let url=window._server+"/static/ureport-asserts/icons/expr-expand-down.svg";
        if(valueType==='dataset'){
            url=window._server+"/static/ureport-asserts/icons/expand-down.svg";
        }
        $td.prepend(`<image src="${url}"></image>`);
    }else if(cellDef.expand === 'Right'){
        let url=window._server+"/static/ureport-asserts/icons/expr-expand-right.svg";
        if(valueType==='dataset'){
            url=window._server+"/static/ureport-asserts/icons/expand-right.svg";
        }
        $td.prepend(`<image src="${url}" style="display: block;"></image>`);
    }else{
        if(valueType==='dataset'){
            let url=window._server+"/static/ureport-asserts/icons/property.svg";
            $td.prepend(`<image src="${url}" style="display: inline-block;"></image>`);
        }else if(valueType==='expression'){
            let url=window._server+"/static/ureport-asserts/icons/expression.svg";
            $td.prepend(`<image src="${url}" style="display: inline-block;"></image>`);
        }
    }
    if(cellStyle.align){
